...
import { useUpdateNotam } from "@/modules/notams/hooks/use-update-notam.action";

interface NotamEditDialogProps {
  notam: {
    id: string;
    title: string;
...
export function NotamEditDialog({ notam, onOpenChange, open, onSuccess }: NotamEditDialogProps) {
  const updateNotam = useUpdateNotam({ onSaved: onSuccess });
  const today = new Date().toISOString().split("T")[0];

  const form = useForm<CreateNotamInput & { id: string }>({
    resolver: zodResolver(createNotamSchema.extend({ id: z.string().uuid() })),
    defaultValues: {
      id: "",
      title: "",
      description: "",
  ...
  useEffect(() => {
    if (notam) {
      const expiresAt = notam.expiresAt ? notam.expiresAt.split("T")[0] : null;
      form.reset({
        id: notam.id,
        title: notam.title,
        ...
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
          <div className="space-y-2">
            ...
            <Input
              ...
              {...form.register("title")}
              disabled={updateNotam.isPending}
            />
          ...
          <DialogFooter className="gap-2 pt-2 border-t border-blue-200/60">
            <Button
              type="button"
              variant="outline"
              onClick={() => onOpenChange(false)}
              disabled={updateNotam.isPending}
              className="border-blue-300 bg-white/80 text-blue-700 hover:bg-blue-50 hover:text-blue-800"
            >
              Cancel
            </Button>
            <Button type="submit" disabled={updateNotam.isPending} className="bg-blue-600 hover:bg-blue-700">
              {updateNotam.isPending ? "Saving..." : "Save Changes"}
            </Button>
          </DialogFooter>
        </form>
      </DialogContent>
