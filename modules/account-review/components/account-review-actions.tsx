...
import { useReviewAccount } from "@/modules/account-review/hooks/use-review-account.action";
...
export function AccountReviewActions({
  ...
  const { approve, isExecuting, reject } = useReviewAccount({
      ...
      {rejecting ? (
        <div className="grid gap-3">
          <Textarea
            ...
            placeholder="Add the reason the applicant should fix before resubmitting"
            value={rejectionReason}
          />
          <div className="flex flex-col gap-2 sm:flex-row sm:justify-end">
            <Button
              className="sm:h-8 border-primary-foreground/20 bg-primary-foreground/10 text-primary-foreground hover:bg-primary-foreground/15 hover:text-primary-foreground w-full sm:w-auto"
            ...
            <Button
              className="sm:h-8 w-full sm:w-auto"
        ...
        <div className="flex flex-col gap-2 sm:flex-row sm:justify-end">
          ...
          <Button
            className="sm:h-8 w-full sm:w-auto"
            disabled={isExecuting}
            onClick={() => approve.execute({ requestId })}
            type="button"
          >
            Approve
          </Button>
