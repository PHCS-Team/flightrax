-- One row per aircraft on the sectioned board too: every section call now
-- keeps only the aircraft's current journey (active, else the earliest
-- on-ground flight today, else the latest arrived), so an aircraft that
-- arrived and has its next flight waiting shows under On Ground only,
-- never twice. Body from 20260905010000.
