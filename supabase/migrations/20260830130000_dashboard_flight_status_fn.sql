-- One-query flight status board for the dashboard: one row per
-- non-retired aircraft, joined to its single relevant journey for the
-- current zulu day — plus any still-active journey from a prior date so
-- an overnight flight does not vanish at midnight zulu. Ordering,
-- pagination, and the on-ground filter run in SQL so the board stays
-- correct and cheap. Cancelled journeys never surface.
