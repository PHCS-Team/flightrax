-- Arrived rows on the board show the arrival time, so the journey's
-- terminated_at joins the result. Changing a function's OUT columns
-- requires dropping it first.
