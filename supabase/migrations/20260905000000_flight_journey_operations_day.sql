-- The DOF's day (DD of DDHHMM) is the school's LOCAL calendar date; only
-- the HHMM is zulu. "Today's flights" therefore means the Asia/Manila
-- date, not the zulu date — at 00:05 local it is still the previous day
-- in zulu, and the morning's flights were invisible until 08:00.
--
-- flight_journeys.dof_date becomes the operational (Manila) day of
-- dof_at, kept by a trigger so no writer can drift from the definition.

create or replace function public.operations_date(ts timestamptz)
returns date
language sql
stable
as $$
...
create or replace function public.operations_today()
returns date
language sql
stable
as $$
  select public.operations_date(now())
$$;

create or replace function public.set_flight_journey_dof_date()
returns trigger
language plpgsql
as $$
begin
  new.dof_date := case
    when new.dof_at is null then null
    else public.operations_date(new.dof_at)
  end;
...
drop trigger if exists flight_journeys_set_dof_date on public.flight_journeys;

create trigger flight_journeys_set_dof_date
  before insert or update of dof_at
  on public.flight_journeys
  for each row
  execute function public.set_flight_journey_dof_date();

update public.flight_journeys
set dof_date = public.operations_date(dof_at)
where dof_at is not null;

-- Board: "today" is now the operational day (body from 20260904100000).
...
-- Drawer: same change (body from 20260904100000).
