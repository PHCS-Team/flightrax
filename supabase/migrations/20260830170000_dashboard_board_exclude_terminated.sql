-- Terminated journeys are "standby": the flight is fully closed and the
-- aircraft must leave the board. The board's journey lookup previously
-- only excluded cancelled journeys, so a swept (terminated) journey
-- still surfaced as Arrived — now only scheduled/active/arrived count.
